...
- (UILabel *)lab1
{
    if (!_lab1) {
        ...
        NSString *mobile = [NSString maskMobile:MyUser.mobile];
        _lab1.text = [NSString stringWithFormat:@"验证码将发送到您的手机%@",mobile] ;
...
- (BOOL)checkContent {
    ...
    if (self.xinMMField.text.length > 15) {
        [self showHint:@"密码不能大于15位!"];
        return NO;
    }
    if (self.xinMMField.text.length < 6) {
